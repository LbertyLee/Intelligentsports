...
public class TrainingReportController extends BaseController {
     ...
     *查看心率明细报告
     **/
    @PostMapping("/exportHeartRate/{taskId}/{braceletId}")
    public R<List<HeartRateDetailsVo>> getHeartRate(@PathVariable Long taskId,@PathVariable String braceletId){
        if(log.isInfoEnabled()){
            log.info("TrainingReportController.getHeartRate.taskId={};braceletId={}", taskId,braceletId);
        }
        return R.ok(trainingReportService.getHeartRate(taskId,braceletId));
    }
